    ...
    <label className='flex grow border rounded overflow-hidden'>
      ...
      {maxButton &&
        <button
          ...
          className='bg-gray-100 border-l py-2 px-4 group text-green-700'>
          Max
        </button>
      }
    </label>
...
const AddAssetButton: NextPage<{
  ...
  return (
    <div className='relative'>
      <button
        className='flex text-green-700 py-2 space-x-1 peer items-center disabled:text-gray-400'
        disabled={isDisabled}>
      ...
      <ul className='absolute divide-y bg-white text-sm max-h-64 border rounded shadow overflow-y-auto scale-0 z-50 peer-focus:scale-100 hover:scale-100'>
        {assets.map(([id, quantity]) => (
          <li key={id}>
            <button
              onClick={() => onSelect(id)}
              className='block w-full h-full p-2 hover:bg-green-700 hover:text-white'>
    ...
    <Panel>
      ...
      <footer className='flex p-4 bg-gray-100 items-center'>
        ...
        <nav className='flex justify-end space-x-2'>
          <BackButton className='p-2 rounded text-green-700 border'>Back</BackButton>
          <button
            className='p-2 rounded text-green-700 border'
...
const TransactionReviewButton: NextPage<{
  ...
  return (
    <Link href={getTransactionPath(result.data)}>
      <a className={['text-white bg-green-700', className].join(' ')}>Review Transaction</a>
    </Link>
  )
}
...
const AddressViewer: NextPage<{
  ...
  return (
    <span className='items-center'>
      <span className='text-wrap' style={{whiteSpace: 'normal', wordWrap: 'break-word'}}>{bech32}</span>
      <CopyButton className='p-2 text-sm text-green-700' getContent={() => bech32} ms={500}>
    ...
    <div className={`${className} text-center`}>
      <div className='space-y-1'>
        ...
        <div className='text-wrap'>
          <span><AddressViewer address={address} /></span>
        </div>
      </div>
      <div className='space-y-1'>
